...
public:
	...
	/**
	 *  The object brush over a footprint that already has something in it.
	 *
	 *  A third colour rather than reusing the red, because the two states are not the same
	 *  answer. Red means the geometry refuses — there is no floor, no wall, nowhere to stand.
	 *  This means it will go exactly where you are pointing, on top of what is already there,
	 *  which is sometimes the intention. Painting both red would teach the eye to read red as
	 *  "probably fine", and then the real refusal goes unnoticed.
	 *
	 *  Amber, and under the cursor, because that is where the eye already is. The same news at
	 *  the far end of the status bar is news nobody reads without widening a panel first.
	 */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Brush")
	FMazeLineStyle BrushBlocked = FMazeLineStyle(FLinearColor(1.00f, 0.65f, 0.10f, 1.0f), 2.0f);
